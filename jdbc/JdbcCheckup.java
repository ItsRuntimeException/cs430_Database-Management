...
// for H2, the embedded DB, runs inside the executable, no user or pw needed
//     java -classpath h2.jar;. JdbcCheckup
...
class JdbcCheckup {
	public static void main(String args[]) {
		...
		try {
			...
			dbSys = readEntry(in, "Using Oracle (o), MySql (m) or H2 (h)? ");
		...
		try {
			...
			} else if (dbSys.equals("h")) { // H2 (embedded db)
				connStr = "jdbc:h2:~/test";
				user = "test";
				password = "";
			} else {
				System.out.println("DB abbreviation not recognized, falling back to manual config...");
				user = readEntry(in, "user: ");
				password = readEntry(in, "password: ");
